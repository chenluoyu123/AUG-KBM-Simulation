...
for p_ky in range(num_ky):
    for p_rangex in range(nRange_x):
        for p_rangey in range(nRange_y):
            dirname='parax_'+num2str_xj(Range_x[p_rangex],effnum)+'~paray_'+num2str_xj(Range_y[p_rangey],effnum)+'~ky_'+num2str_xj(ky_item,effnum)
            if setup['icgyro'] == 1:
                ...
                try:
                    if 'AE_FLAG' in inputgen.keys():
                        if inputgen['AE_FLAG'] == 1:
                            ...
                            Qi_data[p_field][p_ky][p_rangex][p_rangey] = sum(mounttree[dirname]['qlflux_ky']['energy'].T[-1][p_field])
                            Ge_data[p_field][p_ky][p_rangex][p_rangey] = 0
                            Gi_data[p_field][p_ky][p_rangex][p_rangey] = 0
                            Pe_data[p_field][p_ky][p_rangex][p_rangey] = 0
                            Pi_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['momentum'].T[-1][p_field][i_ion_species]
                        else:
                            Ge_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['particle'][-1][p_field][-1]
                            Qe_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['energy'][-1][p_field][-1]
                            # Qi_data[p_field][p_ky][p_rangex][p_rangey] = sum(mounttree[dirname]['qlflux_ky']['energy'].T[-1][p_field][0:-1])
                            # Ge_data[p_field][p_ky][p_rangex][p_rangey] = sum(mounttree[dirname]['qlflux_ky']['particle'][-1][p_field][-1])
                            # Pi_data[p_field][p_ky][p_rangex][p_rangey] = sum(mounttree[dirname]['qlflux_ky']['momentum'].T[-1][p_field][0:-1])
                            Gi_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['particle'][-1][p_field][i_ion_species]
                            Qi_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['energy'].T[-1][p_field][i_ion_species-1]
                            Pe_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['momentum'].T[-1][p_field][- 1]
                            Pi_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['momentum'].T[-1][p_field][i_ion_species-1]
                    else:
                        Ge_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['particle'][-1][p_field][-1]
                        Gi_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['particle'][-1][p_field][i_ion_species-1]
                        Qe_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['energy'][-1][p_field][-1]
                        # Qi_data[p_field][p_ky][p_rangex][p_rangey] = sum(mounttree[dirname]['qlflux_ky']['energy'].T[-1][p_field][0:-1])
                        Qi_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['energy'].T[-1][p_field][i_ion_species-1]
                        Pe_data[p_field][p_ky][p_rangex][p_rangey] = mounttree[dirname]['qlflux_ky']['momentum'].T[-1][p_field][-1]
    ...
    for p_ky in range(num_ky):
        ...
        for k in arange(1,7):
            ...
            if idimplt==0:
                ...
                cmd = "contourf(y_scale*Range_y,x_scale*Range_x," + Channel_name[k - 1] + ".isel(field=p_field,field=p_ky),cmap='seismic')"
            exec(cmd)
            colorbar()
