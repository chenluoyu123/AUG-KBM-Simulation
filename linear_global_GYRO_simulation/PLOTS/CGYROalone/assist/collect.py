...
#            ind_theta=find_index(self.ftheta_plot, theta)
    ...
    #     self.getbigfield()
        ...
        for pt in np.arange(len_ind_t_ave):
            phi_arr=phi_cmplx[:,:,self.int_t_ave[pt]]
            apar_arr=apar_cmplx[:,:,self.int_t_ave[pt]]
            # go over each kx0
            for i, kx0_val in enumerate(kx0_arr):
                # kx' = kx - kx0
                kx_prime = kx_arr - kx0_val
                ...
                maxwell_term = 2./betaE*(kx0_val * ky_grid * (kx0_val**2 - 2*kx0_val*kx_grid) * 
                                apar_arr * np.conj(A_prime))
                # calculate the reynold stress
                k_prime_mag = np.sqrt((kx0_val - kx_grid)**2 + ky_grid**2)
                J0_k_prime = j0(k_prime_mag)
                reynold_term = -1.*(kx0_val * ky_grid * (J0_k2 - J0_k_prime) * 
                                phi_arr * np.conj(phi_prime))
                # sum over the kx and ky space
                Maxwell_arr[pt,i] = np.sum(maxwell_term)
                Reynold_arr[pt,i] = np.sum(reynold_term)
        ...
        self.Reynold_stress=Reynold_Stress
    ...
    #     phi_cmplxx = self.kxky_phi[0, :, :, :, :] + 1j * self.kxky_phi[1, :, :, :,:]  # phi_cmplx[i_r,i_theta_plot,i_n,self.ind_t_ave])
        ...
        # do the calculation
        len_ind_t_ave=len(self.ind_t_ave)
        S_k_kp=np.zeros([self.n_r-1,self.n_n],dtype=complex)
        S_k_kp_norm = np.zeros([self.n_r-1, self.n_n], dtype=complex)
        Lamda=np.zeros([self.n_r-1,self.n_n]) # the coupling coefficient
        ind_kx_0=np.where(self.kx == 0)
#        for i_m in np.arange(self.n_r):
        for i_m in np.arange(self.n_r-1):
    ...
    def Energy_transfer_phi_improve(self, i_theta_plot=0, kx_select=0.0, ky_select=0.2):
        # 维度确认：phi_cmplx_t 的维度为 [kx, ky, time]
        phi_cmplx = self.phi_cmplx_t[1:, i_theta_plot, :, :]  # 形状 (n_kx, n_ky, n_time)
        phi_cmplx=phi_cmplx/self.rho
#        dkx=self.kx[1]-self.kx[0]
#        phi_cmplx=phi_cmplx/dkx
        ...
        # 预计算参数
        n_r, n_n = self.n_r-1, self.n_n
        time_indices = self.ind_t_ave
        ind_kx_0 = np.where(self.kx == 0)[0][0]
        
        # 生成网格索引
        m_grid, n_grid = np.mgrid[:n_r, :n_n]
        kxp = self.kx[m_grid]  # 形状 (n_r, n_n)
        kyp = self.ky[n_grid]
        ...
        valid_mask = (np.abs(delta_kx) <= n_r//2-1) & (np.abs(delta_ky) <= n_n)
        
        # 分块处理参数
        block_size = 256  # 根据GPU内存调整 (单位：MB)
        elem_size = phi_cmplx[0,0,0].nbytes if hasattr(phi_cmplx, 'nbytes') else 16
        block_size = min(len(time_indices), int(block_size*1e6/(n_r*n_n*elem_size)))

        # 初始化结果矩阵
        S_k_kp = np.zeros((n_r, n_n))
        phi_k_select = phi_cmplx[ind_kx, ind_ky, time_indices]  # (n_time,)

        for t_start in range(0, len(time_indices), block_size):
            t_end = min(t_start+block_size, len(time_indices))
            t_slice = slice(t_start, t_end)
            t_block = time_indices[t_slice]
                
            # 获取当前时间块的phi数据
            phi_block = phi_cmplx[..., t_block]  # (n_kx, n_ky, block_size)
                
             # 正ky部分处理
            pos_mask = valid_mask & (delta_ky >= 0)
            pos_kx = (ind_kx_0 + delta_kx)[pos_mask]
            pos_ky = delta_ky[pos_mask]
            if pos_mask.any():
                phi_k_p = phi_block[m_grid[pos_mask], n_grid[pos_mask]]  # (valid, block_size)
                phi_k_kp = phi_block[pos_kx, pos_ky]  # (valid, block_size)
                contrib = np.conj(phi_k_select[t_slice]) * phi_k_p * phi_k_kp
                np.add.at(S_k_kp, (m_grid[pos_mask], n_grid[pos_mask]), contrib.real.mean(axis=1))

            # 负ky部分处理 (共轭)
            neg_mask = valid_mask & (delta_ky < 0)
            neg_kx = (ind_kx_0 - delta_kx)[neg_mask]
            neg_ky = -delta_ky[neg_mask]
            if neg_mask.any():
                phi_k_p = phi_block[m_grid[neg_mask], n_grid[neg_mask]]  # (valid, block_size)
                phi_k_kp = np.conj(phi_block[neg_kx, neg_ky])  # (valid, block_size)
                contrib = np.conj(phi_k_select[t_slice]) * phi_k_p * phi_k_kp
                np.add.at(S_k_kp, (m_grid[neg_mask], n_grid[neg_mask]), contrib.real.mean(axis=1))

        # 应用最终系数
        ipccw = self['input.cgyro.gen']['IPCCW']
        S_k_kp *= -ipccw / len(time_indices)
        self.kx_select_incode=kx_select_incode
        self.ky_select_incode=ky_select_incode
        T_phi=Lamda*S_k_kp
        self.Lamda_phi=Lamda                                                        # coupling coefficient
...
#        self.S_k_kp_norm_phi =S_k_kp_norm                                           # normalized Bicoherence
    ...
    #     phi_cmplxx = self.kxky_phi[0, :, :, :, :] + 1j * self.kxky_phi[1, :, :, :,:]  # phi_cmplx[i_r,i_theta_plot,i_n,self.ind_t_ave])
        phi_cmplx = self.phi_cmplx_t[1:,i_theta_plot,:,:]
        phi_cmplx=phi_cmplx/self.rho
        p_cmplxx = self.kxky_e[0,1:, :, :, :, :] + 1j * self.kxky_e[1, 1:, :, :,:, :]  # p_cmplxx[i_r,i_theta_plot,i_s,i_n,self.ind_t_ave])
        p_cmplx = p_cmplxx[:,i_theta_plot, i_s, : ,:]  # i_s for i_species
        p_cmplx=p_cmplx/self.rho
        # find the the index of the kx_select and ky_select
...
#        for i_m in np.arange(self.n_r):
    ...
    def Energy_transfer_p_improve(self, i_theta_plot=0, i_s=0, kx_select=0.0, ky_select=0.2):
        # 数据预处理 (处理五维结构)
        phi_cmplx = self.phi_cmplx_t[1:, i_theta_plot, :, :]  # [kx, ky, time]
        phi_cmplx=phi_cmplx/self.rho
        p_cmplx = self.kxky_e[0, 1:, i_theta_plot, i_s, :, :] + 1j * self.kxky_e[1, 1:, i_theta_plot, i_s, :, :]  # [kx, ky, time]
        p_cmplx=p_cmplx/self.rho
        # 向量化索引查找
        kx_idx = np.abs(self.kx - kx_select).argmin()
        kx_select_incode = self.kx[kx_idx]
        ky_idx = np.abs(self.ky - ky_select).argmin()
        ky_select_incode = self.ky[ky_idx]
        ...
        # 生成全局网格索引
        m_grid, n_grid = np.mgrid[:n_r, :n_n]
        kxp_all = self.kx[m_grid]  # 形状 [n_r, n_n]
        kyp_all = self.ky[n_grid]
    
        # 向量化计算Lamda矩阵
        Lamda = kx_select_incode * kyp_all - kxp_all * ky_select_incode
    
        # 预计算有效索引
        delta_kx = kx_idx - m_grid
        delta_ky = ky_idx - n_grid
        valid_mask = (np.abs(delta_kx) <= n_r//2-1) & (np.abs(delta_ky) <= n_n)
    
        # 分块处理参数 (自动调整块大小)
        mem_per_element = 16  # complex128: 16 bytes
        block_size = min(total_time, int(2**28 // (n_r*n_n*mem_per_element)))  # 256MB块
        print(f"Using block size: {block_size} time steps")
    
        # 初始化累加器
        S_k_kp = np.zeros((n_r, n_n), dtype=np.complex128)
        norm_accumulator = np.zeros((n_r, n_n, 3))  # 存储 |p_k|, |p_p|, |phi|
        ...
        for t_start in range(0, total_time, block_size):
            ...
            # 加载当前时间块数据
            phi_block = phi_cmplx[..., time_indices[t_slice]]  # [kx, ky, block]
            p_block = p_cmplx[..., time_indices[t_slice]]      # [kx, ky, block]
            p_k_select = p_block[kx_idx, ky_idx, :]            # [block]
            ...
            if pos_mask.any():
                # 提取相关数据块
                p_p = p_block[m_grid[pos_mask], n_grid[pos_mask]]      # [valid, block]
                phi_part = phi_block[pos_kx, pos_ky]                   # [valid, block]
                
                # 计算贡献值
                contrib = np.conj(p_k_select) * p_p * phi_part         # [valid, block]
                np.add.at(S_k_kp, (m_grid[pos_mask], n_grid[pos_mask]), contrib.sum(axis=1))
                
                # 累加归一化因子
                norm_accumulator[m_grid[pos_mask], n_grid[pos_mask], 0] += np.abs(p_p).sum(axis=1)
                norm_accumulator[m_grid[pos_mask], n_grid[pos_mask], 2] += np.abs(phi_part).sum(axis=1)
            ...
            if neg_mask.any():
                # 提取相关数据块（带共轭）
                p_p = p_block[m_grid[neg_mask], n_grid[neg_mask]]      # [valid, block]
                phi_part = np.conj(phi_block[neg_kx, neg_ky])         # [valid, block]
                
                # 计算贡献值
                contrib = np.conj(p_k_select) * p_p * phi_part         # [valid, block]
                np.add.at(S_k_kp, (m_grid[neg_mask], n_grid[neg_mask]), contrib.sum(axis=1))
            
                # 累加归一化因子
                norm_accumulator[m_grid[neg_mask], n_grid[neg_mask], 0] += np.abs(p_p).sum(axis=1)
                norm_accumulator[m_grid[neg_mask], n_grid[neg_mask], 2] += np.abs(phi_part).sum(axis=1)
        
            # 累加|p_k|因子
            norm_accumulator[:, :, 1] += np.abs(p_k_select).sum()
    
        # 后处理
        S_k_kp_real = np.real(S_k_kp) / total_time
    
        # 计算归一化因子
        with np.errstate(divide='ignore', invalid='ignore'):
            norm_factors = (norm_accumulator[:, :, 1] *  # |p_k|
                            norm_accumulator[:, :, 0] *  # |p_p|
                            norm_accumulator[:, :, 2])   # |phi|    
            norm_factors /= total_time**3
            S_k_kp_norm = np.divide(S_k_kp_real, norm_factors, where=norm_factors!=0)
    
        # 应用系数
        ipccw = self['input.cgyro.gen']['IPCCW']
        S_k_kp_real *= -ipccw
        S_k_kp_norm *= -ipccw
        T_p = Lamda * S_k_kp_real
    
        # 存储结果
        self.Lamda_p = Lamda
        self.S_k_kp_p = S_k_kp_real
        self.S_k_kp_norm_p = S_k_kp_norm
        self.T_p = T_p
        self.kx_select_incode=kx_select_incode
        self.ky_select_incode=ky_select_incode
