...
class OMFITgyro_eigen(OMFITgyro):
    ...
    def __init__(self,filename=None,extra_files=[],test_mode=False,theta_p=np.linspace(-np.pi,np.pi,37),ireadflag=0):
        ...
        self.boundary_method=inputgyro['BOUNDARY_METHOD']
...
# #            self.epar_ave=epar_ave
    ...
    def turn_thetap2thetab(self,value):
        ...
        if self.boundary_method==1:
            ...
            value_b=np.zeros([int(self.n_r/2),self.n_theta])
            for k in np.arange(0,int(self.n_r/2)):
                value_b[k,:]=value_bb[0:-1]
            value_b=np.reshape(value_b,[int(self.n_r/2)*self.n_theta]) 
        return value_b
